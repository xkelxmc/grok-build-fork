...
fn collect_post_tool_use_json(stdout: &str) -> Vec<PostToolUseHookJson> {
    ...
    while let Some(rel) = stdout[index..].find('{') {
        let start = index + rel;
        let mut stream =
            serde_json::Deserializer::from_str(&stdout[start..]).into_iter::<PostToolUseHookJson>();
        match stream.next() {
            Some(Ok(json)) => {
...
fn merge_post_tool_use_json(
    ...
    combined.outcome.additional_context =
        (!contexts.is_empty()).then(|| clip_text(&contexts.join("\n\n"), MAX_HOOK_FEEDBACK_CHARS));
    combined
}
